...
# URLs
URL_PACT = 'https://pactwebserial.wordpress.com/table-of-contents/'
URL_WARD = 'https://www.parahumans.net/table-of-contents/'
...
def LinksDosCapitulos(arco, url, LIVRO):
    r = requests.get(url)
    b = BeautifulSoup(r.text, "html.parser")
    ...
    for link in links:
        ...
        if LIVRO == '2':
            href = f'https://parahumans.net{href}'
...
def LerConteudo(links, conteudoCapitulos):
    for url in links:
        ...
        r = requests.get(url)
        r.encoding = 'utf-8'        # Define UTF-8 explicitamente
        b = BeautifulSoup(r.text, "html.parser")
...
def create_epub(book_title, capitulos):
    ...
    epub.write_epub(f'{book_title}.epub', book, {"encoding": "utf-8"})
...
LIVRO = input()
if not LIVRO:
    print('Erro, rode o programa novamente')
    url = 0
elif(LIVRO == '1'):
    url = URL_PACT
elif(LIVRO == '2'):
...
for arco in range(inicio, fim + 1):
    ...
    LerConteudo(LinksDosCapitulos(arco, url, LIVRO), conteudoCapitulos)
